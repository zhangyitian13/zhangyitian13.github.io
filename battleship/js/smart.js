...
function __Hash()
{
	...
	this.check = function(status)
	{
		...
		for(var i = f % this.length ; i < this.length ; i++)
		{
			if(!this.container[i])
			{
				this.container[i] = f;
...
function __Smart()
{
	...
	this.query = function (status , target)
	{
		...
		for( ; ; )
		{
			...
			//debugger;
			//console.log(closed);
			if(!closed || closed.dist < 0.2 && closed.angularVelocity == 0 || limit > m_refresh_interval * 30)
			{
			...
			{
				if(closed.angularVelocity == MaxAngularVelocity || closed.angularVelocity == -MaxAngularVelocity || closed.angularVelocity == 0)
					if(hash.check({
				...
				if(closed.angularVelocity != MaxAngularVelocity)
					if(hash.check({
									pos_x : Math.floor(closed.pos_x + dx) , 
									pos_y : Math.floor(closed.pos_y + dy) , 
									orientation : (closed.orientation + closed.angularVelocity + 24) % 24 ,
